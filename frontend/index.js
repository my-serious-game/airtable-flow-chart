    ...
    if (!queryResult.isDataLoaded) {
      ...
      createLayout(settingsValidationResult.settings).then((svg) => {
        ...
        if (svgElement && graph.current) {
          ...
          // app frame.
          const width = svgElement.getAttribute("width").replace("pt", "");
          const height = svgElement.getAttribute("height").replace("pt", "");
          if (Number(width) > 822 || Number(height) > 595) {
            const divisor = Number(width) / 2000;
            svgElement.setAttribute("width", `${Number(width) / divisor}pt`);
            svgElement.setAttribute("height", `${Number(height) / divisor}pt`);
          } else {
    ...
    if (view && graph.current) {
      ...
      if (svgElement) {
        if (exportType === ExportType.PNG) {
          saveSvgAsPng(
            svgElement,
            `${view.name}-${new Date().toDateString()}.png`,
            {
              scale: 2.0,
            }
          );
